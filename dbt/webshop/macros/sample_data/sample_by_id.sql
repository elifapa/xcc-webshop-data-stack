...
{% macro sample_by_id_fixed_rows(indexed_column) -%}
    ...
    {% if target.name != sample_target_output %}
        {{ log("Skipping sample table creation because target is not " ~ sample_target_output, info=True) }}
        {% do return(None) %}
    {% endif %}
    ...
    {%- for source in sources -%}
        ...
        {% do log("Sampled table " ~ target.schema ~ "." ~ source ~ " is created! Rejoice!", info=True) %}
...
{% macro sample_by_id_variable_rows(indexed_column) -%}
    ...
    {%- for source in sources -%}
        {% set sampled_row_count = [(sources_row_counts[source] * var('sample_size_ratio')) | round | int, 5] | max %}

        {% do log("Creating sampled table " ~ source ~ "...", info=True) %}

        {% do log("Row count: " ~ sources_row_counts[source] ~ " | Sampled row count: " ~ sampled_row_count, info=True) %}
